...
const headerClassName =
  "flex w-full items-center justify-between gap-5 rounded-full border border-[rgba(219,228,240,0.95)] bg-white/92 px-4 py-3 shadow-[0_1px_2px_rgba(15,23,42,0.04)] backdrop-blur-[14px] max-[900px]:flex-wrap max-[900px]:gap-3 max-[900px]:rounded-[24px] max-md:flex-nowrap max-md:gap-3 max-md:p-[12px]";
      ...
      <header className={headerClassName}>
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="flex min-w-0 flex-col">
            <span className="truncate text-[18px] font-bold uppercase tracking-[0.04em] text-slate-900">
              TaskFurnace
            </span>
            <span className="hidden min-w-0 truncate whitespace-nowrap text-xs text-slate-500 min-[420px]:block max-[900px]:hidden max-md:block">
              Always-on orchestration shell
            </span>
          </div>
        </div>
        <nav className="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-100 p-1 max-md:gap-1">
          <button
            type="button"
